...
__all__ = ["main", "parse_args", "device_os_choices"]
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--settings",
        type=Path,
        help="Path to settings YAML (defaults to settings.yaml next to this package)",
    )
    parser.add_argument(
        "--rules",
        type=Path,
        help="Path to rules YAML (defaults to rules.yaml next to this package)",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
...
def _build_sample_inventory(config: AppConfig) -> NormalizedInventory:
