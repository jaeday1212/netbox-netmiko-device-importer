...
def load_app_config(
    ...
    token_env = netbox_section.get("token_env")
    ...
    if not token:
        if allow_missing_token:
            ...
            raise ValueError("NetBox API token must be provided via settings or environment variable")
    ...
    interface_rules = rules_data.get("interface_types", {
        "physical_default": "other",
        "lag_default": "lag",
        "matches": [],
    })

    device_type_suffix = rules_data.get("device_type_suffix", {"enabled": False, "value": ""})
